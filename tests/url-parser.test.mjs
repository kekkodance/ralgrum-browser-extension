...
describe("Deezer pages", () => {
  it("parses supported bare and locale URLs", () => {
    assert.equal(
      detectors.parseDeezerUrl("https://www.deezer.com/us/track/3135556").id,
      "3135556",
    );
    assert.equal(
      detectors.parseDeezerUrl("https://www.deezer.com/track/3135556").type,
      "track",
    );
    assert.equal(
      detectors.parseDeezerUrl("https://listen.deezer.com/album/302127").type,
      "album",
    );
  });
  ...
  it("parses album, playlist, and artist entity paths", () => {
    assert.equal(
      detectors.parseDeezerUrl("https://www.deezer.com/album/302127").type,
      "album",
    );
    assert.equal(
      detectors.parseDeezerUrl("https://www.deezer.com/playlist/13743145521")
        .type,
      "playlist",
    );
    assert.equal(
      detectors.parseDeezerUrl("https://www.deezer.com/artist/27").type,
      "artist",
    );
  });
  ...
  it("requires HTTPS, an allowed host, and one exact entity path", () => {
    assert.equal(
      detectors.parseDeezerUrl("http://www.deezer.com/track/3135556"),
      null,
    );
    assert.equal(
      detectors.parseDeezerUrl("https://notdeezer.com/track/3135556"),
      null,
    );
    assert.equal(
      detectors.parseDeezerUrl("https://deezer.com.evil.example/track/3135556"),
      null,
    );
    assert.equal(
      detectors.parseDeezerUrl("https://deezer.com.br/album/302127"),
      null,
    );
    assert.equal(
      detectors.parseDeezerUrl("https://www.deezer.com.us/artist/27"),
      null,
    );
    assert.equal(
      detectors.parseDeezerUrl("https://www.deezer.com/us/fr/track/3135556"),
      null,
    );
    assert.equal(
      detectors.parseDeezerUrl("https://www.deezer.com/track/0"),
      null,
    );
    assert.equal(
      detectors.parseDeezerUrl("https://www.deezer.com/search?q=test"),
      null,
    );
  });
  ...
  it("accepts 32-digit IDs but rejects zero, leading zeroes, and longer IDs", () => {
    ...
    assert.equal(
      detectors.parseDeezerUrl(`https://www.deezer.com/track/${longestId}`).id,
      longestId,
    );
    for (const id of ["0", "01", "1".repeat(33)]) {
      assert.equal(
        detectors.parseDeezerUrl(`https://www.deezer.com/track/${id}`),
        null,
        id,
      );
    }
...
describe("SoundCloud pages", () => {
  it("detects artist, track, playlist, and album shapes", () => {
    assert.equal(
      detectors.parseSoundcloudUrl("https://soundcloud.com/someartist").type,
      "artist",
    );
    assert.equal(
      detectors.parseSoundcloudUrl(
    ...
    assert.equal(
      detectors.parseSoundcloudUrl(
        "https://soundcloud.com/someartist/sets/some-mix",
      ).type,
      "playlist",
    );
    assert.equal(
      detectors.parseSoundcloudUrl(
        "https://soundcloud.com/someartist/albums/some-album",
      ).type,
      "album",
    );
  });
  ...
  it("requires HTTPS and rejects every reserved top-level route", () => {
    ...
    for (const route of reserved) {
      assert.equal(
        detectors.parseSoundcloudUrl(`https://soundcloud.com/${route}`),
        null,
        route,
      );
    }
    ...
    assert.equal(
      detectors.parseSoundcloudUrl("https://soundcloud.com.evil.example/a/b"),
      null,
    );
...
describe("ralgrum links", () => {
  ...
  it("enforces Deezer IDs even when callers bypass URL detection", () => {
    ...
    for (const id of [undefined, null, 0, "01", "1".repeat(33), "-1", "abc"]) {
      assert.equal(
        detectors.buildRalgrumUrl({ ...entity, id }),
        null,
        String(id),
      );
    }
    ...
    assert.equal(
      new URL(detectors.buildRalgrumUrl({ ...entity, id: 1 })).searchParams.get(
        "id",
      ),
      "1",
    );
  ...
  it("keeps the longest whole-emoji title prefix that fits the wire limit", () => {
    ...
    const maxEmoji = Math.floor(
      (2048 - Buffer.byteLength(required) - "&title=".length) / 12,
    );
  ...
  it("caps titles at 200 Unicode scalars rather than UTF-16 units", () => {
    ...
    assert.equal(
      new URL(link).searchParams.get("title"),
      "a".repeat(199) + "😀",
    );
  });
  ...
  it("preserves URLSearchParams Unicode, literal plus, and replacement semantics", () => {
    const entity = detectors.parseSoundcloudUrl(
      "https://soundcloud.com/a/b?tag=a+b&encoded=%2B",
    );
    const link = detectors.buildRalgrumUrl(entity, {
      title: "\uD800 A+B &/% é😀",
    });
    ...
    assert.equal(params.get("url"), entity.url);
    // eslint-disable-next-line no-control-regex -- asserting the built link stays pure ASCII
  ...
  it("omits an unfit optional title and rejects unfit required fields without shortening the URL", () => {
    const entity = detectors.parseDeezerUrl(
      "https://www.deezer.com/track/1?ref=",
    );
    const requiredLength = Buffer.byteLength(detectors.buildRalgrumUrl(entity));
    ...
    assert.equal(
      detectors.buildRalgrumUrl(entity, { title: "Optional title" }),
      null,
    );
  });
});
