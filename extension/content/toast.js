...
(function (root) {
  ...
  var SHOWN = {};
  var DEEZER_PATH = 'M14.8 101.1C6.6 101.1 0 127.6 0 160.3s6.6 59.2 14.8 59.2 14.8-26.5 14.8-59.2-6.6-59.2-14.8-59.2zM448.7 40.9c-7.7 0-14.5 17.1-19.4'
  ...
  var SOUNDCLOUD_PATH = 'M640.2 298.6c-1.3 23.1-11.5 44.8-28.4 60.5s-39.2 24.4-62.3 24.1l-218 0c-4.8 0-9.4-2-12.8-5.4s-5.3-8-5.3-12.8l0-234.8c-.2-4 .9-8 3.1-11.4s5.3-6.1 9-7.7c0 0'
  ...
  var XMARK_PATH = 'M55.1 73.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L147.2 256 9.9 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192.5 301.3 329.9 438.6c12.5 12.5 32.8'
    + '12.5 45.3 0s12.5-32.8 0-45.3L237.8 256 375.1 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192.5 210.7 55.1 73.4z';
  var MUSIC_PATH = 'M468 7c7.6 6.1 12 15.3 12 25l0 304c0 44.2-43 80-96 80s-96-35.8-96-80 43-80 96-80c11.2 0 22 1.6 32 4.6l0-116.7-224 49.8 0 206.3c0 44.2-43 80-96 80s-96-35.8-96-80'
  ...
  function musicGlyph() {
    return '<svg width="14" height="14" viewBox="0 0 512 512" aria-hidden="true">'
      + '<path fill="' + MUTED + '" d="' + MUSIC_PATH + '"/>'
      + '</svg>';
  }
  function xGlyph() {
    return '<svg width="12" height="12" viewBox="0 0 384 512" aria-hidden="true">'
      + '<path fill="' + MUTED + '" d="' + XMARK_PATH + '"/>'
      + '</svg>';
  }
  function ensureHost() {
  ...
  function styleText() {
    return ''
      ...
      + '.rg-head{display:flex;align-items:center;gap:8px;padding:8px 12px;border-bottom:1px solid ' + BORDER + ';background:' + CHROME_BG + ';}'
      ...
      + '.rg-brand{margin:0;font-size:13px;line-height:1;font-weight:600;color:' + FOREGROUND + ';}'
      ...
      + '.rg-trow{display:flex;align-items:center;gap:9px;padding:7px 8px;border-radius:6px;'
      + 'border:1px solid transparent;cursor:pointer;}'
      + '.rg-trow:hover{background:' + ROW_HOVER_BG + ';border-color:' + BORDER + ';}'
      + '.rg-art{width:40px;height:40px;flex:none;object-fit:cover;border-radius:5px;border:1px solid ' + BORDER + ';background:' + ART_BG + ';display:block;}'
      ...
      + '.rg-ttitle{margin:0;flex:1;min-width:0;font-size:12.5px;font-weight:500;color:' + FOREGROUND + ';'
      + 'white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}'
      + '.rg-tartist{margin:1px 0 0;font-size:11.5px;color:' + MUTED + ';'
      + 'white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}'
      + '.rg-explicit{position:relative;top:2px;width:14px;height:14px;flex:none;display:inline-flex;align-items:center;justify-content:center;'
      + 'border-radius:3px;border:1px solid rgba(239, 68, 68, 0.4);background:rgba(239, 68, 68, 0.15);'
      + 'font-size:8px;font-weight:800;color:' + DANGER + ';}'
      + '.rg-prov{flex:none;display:flex;align-items:center;margin-right:2px;}'
      + '.rg-hrow{display:flex;align-items:center;gap:16px;}'
      + '.rg-hart{width:76px;height:76px;flex:none;object-fit:cover;border-radius:6px;border:1px solid ' + BORDER + ';background:' + ART_BG + ';display:block;}'
      ...
      + 'color:' + FOREGROUND + ';font-family:' + FONT + ';font-size:13px;line-height:1;font-weight:600;cursor:pointer;'
      ...
      + '.rg-secondary{height:34px;padding:0 11px;border-radius:6px;border:1px solid ' + BORDER + ';background:transparent;'
      + 'color:' + FOREGROUND + ';font-family:' + FONT + ';font-size:13px;line-height:1;font-weight:500;cursor:pointer;white-space:nowrap;'
  ...
  function centerTrackHtml(entity, meta) {
    ...
    var badgeHtml = meta && meta.explicit
      ? '<span class="rg-explicit">E</span>'
      : '';
    ...
    return '<div class="rg-trow" data-rg-play="1" role="button" aria-label="Play in ralgruM">'
      + artHtml
      + '<div class="rg-tcol">'
      + '<div class="rg-ttitle-row"><p class="rg-ttitle">' + esc(title) + '</p>' + badgeHtml + '</div>'
      + artistHtml
      + '</div>'
  ...
  function centerHeaderHtml(entity, meta) {
    ...
    var artHtml = artwork
      ...
      : '<span class="rg-hart-fallback"></span>';
  ...
  function hasTitle(meta) {
    return !!(meta && cleanText(meta.title));
  }
  ...
  function showOncePerPage(entity, meta, related) {
    ...
    var seen = SHOWN[key];
    if (!seen) {
      SHOWN[key] = { titled: hasTitle(meta) };
      return show(entity, meta, related);
    }
    if (!seen.titled && hasTitle(meta)) {
      seen.titled = true;
      return show(entity, meta, related);
    }
    return false;
