...
(function (root) {
  ...
  function ralgrumUrlFor(entity, action, title) {
    if (root.RalgrumDetectors && root.RalgrumDetectors.buildRalgrumUrl) {
      return root.RalgrumDetectors.buildRalgrumUrl(entity, {
        action: action,
        title: title,
      });
    }
  ...
  function createHeader() {
    ...
    header.appendChild(
      logo
        ? createImage("rg-logo", logo)
        : createElement("span", "rg-logo-fallback", "R"),
    );
  ...
  function createTrackRow(entity, meta) {
    var title =
      cleanText((meta && meta.title) || entity.title) || "Unknown track";
  ...
  function createCollectionHeader(entity, meta) {
    var title =
      cleanText((meta && meta.title) || entity.title) || typeName(entity.type);
    var sub = cleanText(meta && meta.subtitle);
    var artwork = (meta && meta.artwork) || "";
    var row = createElement("div", "rg-hrow");
    row.appendChild(
      artwork
        ? createImage("rg-hart", artwork)
        : createElement("span", "rg-hart-fallback"),
    );
  ...
  function createFooter(entity, related, primaryUrl) {
    ...
    var mainLabel =
      entity.type === "track" ? "Play in ralgruM" : "Open in ralgruM";
  ...
  function render(entity, meta, related, update) {
    ...
    var content =
      entity.type === "track"
        ? createTrackRow(entity, meta)
        : createCollectionHeader(entity, meta);
  ...
  function showOncePerPage(entity, meta, related) {
    ...
    if (seen.shownTitle !== null || seen.shownSubtitle !== null) {
      ...
      var visualChanged =
        seen.artwork !== previousArtwork || seen.explicit !== previousExplicit;
    ...
    if (update) {
      render(
        seen.entity,
        {
          ...
          artwork: seen.artwork,
          explicit: seen.explicit,
        },
        seen.related,
        true,
      );
  ...
  function armSettle(seen) {
    if (
      seen.settleTimer ||
      seen.shownTitle !== null ||
      seen.shownSubtitle !== null
    ) {
      return;
    }
    clearDeadline(seen);
